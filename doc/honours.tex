...
\subsection{Sheaf of $k$-algebras}
\begin{definition}
...
Let $V \subseteq \AA^n$ be an algebraic set.
Let us see how to endow $V$ with the structure of a ringed space.
Recall that the basic open subsets of $V$ are $\{D(h) : h \in k[V]\}$, where $D(h) := \{P \in V : h(P)\ne 0\}$.
A pair of polynomials $g, h \in k[V]$ with $h \ne 0$ defines the function $\varphi : D(h) \to k$, given by $P \mapsto \frac{g(P)}{h(P)}.$
